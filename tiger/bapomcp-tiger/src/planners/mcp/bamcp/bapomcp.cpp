		...
		if(!Params.BANDIT){
			...
			for(uint c=0;c<SAS;++c){
        		...
        		counts1[c]=10000;
      			...
      			counts1[0] = int(utils::rng.rand_closed01()*10);
      			counts1[1] = int(utils::rng.rand_closed01()*10);
      			counts1[SA] = int(utils::rng.rand_closed01()*10);
      			counts1[SA+1] = int(utils::rng.rand_closed01()*10);
...
bool BAPOMCP::Update(uint& ssnew , uint ss, uint aa, uint observation, double reward)//update the belief
{
...
void BAPOMCP::UCTSearch()//Search the best action(BAPOMCP)
{
...
int BAPOMCP::GreedyUCB(VNODE* vnode, bool ucb) const//choose an action
{
...
bool BAPOMCP::AddTransforms(VNODE* root, BELIEF_STATE& beliefs)//update the particle
{
    int attempts = 0, added = 0,maxattempts=0;
    double probsum=0;
   
    while (added < 1000&&maxattempts<1000*100)
    {
