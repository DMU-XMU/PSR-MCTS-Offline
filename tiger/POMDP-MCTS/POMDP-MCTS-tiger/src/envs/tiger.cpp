...
//		O2[11]=0.649046;
	...
	int o = 0;
	int aflag = 0;
	std::ifstream in("POMDP1");
	std::ofstream out("POMDP1-out.txt");
	for(double x; in >> x;)
	{
		if(a >= aflag)
		{
			T2[s1*S*A+a*S+s2] = x;
			out << "T2 " << s1*S*A+a*S+s2 << " " <<  x << " ";
			s2 = s2+1;
			if(s2 == 2)
			{
				s2 =0;
...
bool Tiger::StepPOMDPsim(STATE& state, uint action, uint& observation, double& immediateReward)const
{
	...
	uint stateold = tigerstate.position;
	immediateReward = R[stateold*A+action];
	//std::cout << "stateold = " << stateold << "  action = " << action << std::endl;
	//std::cout << "O2[stateold*6+action*2] = " << O2[stateold*6+action*2] << std::endl;
	//std::cout << "T2[stateold*6+action*2] = " << T2[stateold*6+action*2] << std::endl;
	if(action == 0){
		if (utils::rng.rand_closed01()<=O2[stateold*6+action*2]) {
			observation=0;
		}
		else{
			observation=1;
		...
		if (utils::rng.rand_closed01()<T2[stateold*6+action*2]) {
			statenew=0;
		}
		else{
			statenew=1;
		...
		if (utils::rng.rand_closed01()<=O2[statenew*6+action*2]) {
			observation=0;
		}
		else{
			observation=1;
...
//	
//	//std::cout<<"sample"<<std::endl;
//	utils::rng.multinom(T2+tigerstate.position*S*A+action*S,S,statenew);
