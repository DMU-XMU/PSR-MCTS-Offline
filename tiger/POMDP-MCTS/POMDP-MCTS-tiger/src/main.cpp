...
#include "planners/mcp/experiment_bapomcp.h"
#include "planners/mcp/bamcp/bamcp.h"
#include "planners/mcp/bamcp/bapomcp.h"
...
int main(int argc, char* argv[]){
	...
	if(BAPOMCP){
		std::cout << std::endl << "**************" << std::endl;
		std::cout << "--BAPOMCP-- K " << K << " EC " << MCTSEC << std::endl;
		Rhist = std::vector<std::vector<double> >(N);;

		BAPOMCP::PARAMS searchParamsBAMCP;
		EXPERIMENTBAPOMCP::PARAMS expParamsBAMCP;
		searchParamsBAMCP.MaxDepth = real->GetHorizon(expParamsBAMCP.Accuracy, expParamsBAMCP.UndiscountedHorizon);
		expParamsBAMCP.NumSteps = T;
		searchParamsBAMCP.NumSimulations = K;
		expParamsBAMCP.AutoExploration = true;
		expParamsBAMCP.TimeOut = 32000;
		searchParamsBAMCP.ExplorationConstant = MCTSEC;
		searchParamsBAMCP.RB = MCTSRB;
		searchParamsBAMCP.eps = MCTSEPS;
		std::string filename_bmcp_all(datafilename);
		filename_bmcp_all.append("_bpomcp_all");
		EXPERIMENTBAPOMCP experiment7(*real, *real,filename_bmcp_all, 
				expParamsBAMCP,
				searchParamsBAMCP,
				*samplerFact);
...
//		expresult<<"mean:";
//		expresult<<mean/N<<" ";
//		resultrucc <<mean/N<<std::endl;
//		timer.stopTimer();

		timer.startTimer();
		//int num = 400;
		//double *RESU =new double[400];
		double RESU[400] = {0}; 
		...
		for(uint i=0;i<N;++i){
			...
			//RESU[i] = mean;
			//std::cout << "mean"<< mean/400 << std::endl;
		...
		double mean = 0;
		int count_nonan = 400;
		std::ofstream meanresult("result_t.txt");
		for(int k=0;k<400;k++){
		    meanresult << RESU[k]/N << std::endl;
		    //std::cout<< RESU[k]/N << std::endl;
		    if (std::isnan((RESU[k]/N)))
		    {	std::cout<< "a nan!!!!!!!!!!!!!!!!!" << std::endl;
			count_nonan--;
		    }
		    else
		    {
		    ...
		    //std::isnan(RESU[k]/N) ? count_nonan-- : mean+=RESU[k]/N;
		//RESU[k]=RESU[k]/N;
		
		//std::cout<<RESU[k]<<std::endl;
		//expresult<<RESU[k]/400<<" ";
		}
		meanresult << "total: " <<mean << std::endl;
		mean =mean/count_nonan;
		
		meanresult << "nonancount: " <<count_nonan  << std::endl;
		meanresult << "mean 400 result: " <<mean  << std::endl;
		//std::cout<<mean<<std::endl;		
		//std::cout<<mean/400<<std::endl;		
		timer.stopTimer();
		std::string filename_bmcp(datafilename);
		filename_bmcp.append("_bpomcp");
		utils::dumpc(Rhist,filename_bmcp,400);
		std::cout << "Time: " << timer.getElapsedTime()/(double)N  << " s";
		utils::append(timer.getElapsedTime()/(double)N,filename_bmcp_all);
	}
  if(BAUCT){
