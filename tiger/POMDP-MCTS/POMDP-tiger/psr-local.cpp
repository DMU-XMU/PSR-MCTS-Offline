...
{   
  	...
  	//vector<symList*>* divide = env->getData5(belief, 10, 100);
  	vector<symList*>* divide = env->getData5(belief, 200, 6);
  	int S = 2, O = 2, A = 3;
	...
	//验证模型误差
  	//给定初始状态分布(0.5,0.5)执行算出所有一步检验概率，求出两个模型分布的欧式距离
...
//  		
//  		modelpro = getabstractBeliefprob(RandomPOMDP,belief_start,*s,S);
//  		
...
//     POMDPTXT<<"T矩阵为"<<":"<<endl;         
     for(int i=0;i<size_newS;i++) 
        {  
         for(int j=0;j<size_newS;j++)
            { 
            	if(isnan((*RandomPOMDP->TMatrix)[a][i][j])==true)
            		(*RandomPOMDP->TMatrix)[a][i][j] = 0;
