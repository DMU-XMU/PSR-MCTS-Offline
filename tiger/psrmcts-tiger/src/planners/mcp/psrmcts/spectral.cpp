...
symList2IntMap* SPECTRAL::createPsrdata2(int num_Data)
{
	...
	for(int j=0;j<num_Data;j++)
	{
		...
		//actionToSend=getAction(Simulator.SelectRandom());
	...
	int size=continueData->size();                    
  	//cout<<"Data->size()="<<continueData->size()<<endl;
	...
	for(s=landmarkSet->begin();s!=landmarkSet->end();s++)
	{
 	 ...
 	 for(int pos=0;pos<size-1;pos+=12) 
  	{  
    	symList *currData= new symList();
		for(int pos1=pos;pos1<pos+12;pos1+=2)
		{
  ...
  Histcout=getHistcout1(Matrix,1,TXTNAME);
...
{    
  ...
  s2 = 0; t = 0.0;
  while ((prob > t + Tr[a][s1][s2]*100) && (s2 < S_sz))//get new state
  {
    t += Tr[a][s1][s2]*100; s2++;
  }
