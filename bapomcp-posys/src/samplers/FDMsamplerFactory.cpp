...
Sampler* FDMsamplerFactory::getTigerPOMDPSampler(const uint* counts, uint S, uint A, uint O,
		double* R, bool rsas, double gamma){
	FDMTigerPOMDPSampler* sampler = new FDMTigerPOMDPSampler(counts, alpha, S, A, O, R, rsas, gamma);
	return (Sampler*) sampler;
}
