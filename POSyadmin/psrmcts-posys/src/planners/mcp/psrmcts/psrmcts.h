...
public:
    ...
    double RolloutPSR();
    void UpdatePSR(uint action,uint observation,bool& find);
    void FreeusedRoot();


    uint getPSRresult(uint action,CMatrix* bt);
    uint getPSRresultAOless(uint action,CMatrix* bt);
    ...
    void ConstructSameTree1();
    int SelectActionPSR();


    Array1D<int>* index;
    Array1D<int> * actionIndex;
    Array1D<int> * AOnum;
    void getAOnumByA();
    void getsysAOsetpossibleidInAOset(symSet* AOsetpos,symSet* AOset);
    int getObyAO(int ao);
    void getActBeginIndex();
    int getAOindex(int ao,bool& find);
