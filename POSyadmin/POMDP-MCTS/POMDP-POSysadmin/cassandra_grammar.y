...
%token DISCOUNT VALUES REWARD COST STATES ACTIONS NEWACTIONS NEWOBSERS OBSERVATIONS START
...
newact_param:	NEWACTIONS ':' newact_val ;
newact_val:	INTEGER			{
			/* initialize n actions with default mnemonics */
			toret->nrnewact = $1;
			toret->arr_newact = (char **) malloc($1 * sizeof(char *));
			int i;
			for (i=0; i<$1; i++) {
				toret->arr_newact[i] = (char *) malloc((nrdigit($1,16)+2) * sizeof(char));
				sprintf(toret->arr_newact[i], "d%X", i);
			}
		}
		| newact_list
		;
newact_list:	newact_list STRING		{
			/* add new action with given mnemonic */
			toret->nrnewact++;
			...
			toret->arr_newact[toret->nrnewact-1] = (char *) malloc((strlen($2)+1)*sizeof(char));
			strcpy(toret->arr_newact[toret->nrnewact-1], $2);
			free($2);
		}
		| STRING		{
			/* add new action with given mnemonic */
			...
			free(toret->arr_newact);
			toret->arr_newact = tmpptr;
			toret->arr_newact[toret->nrnewact-1] = (char *) malloc((strlen($1)+1)*sizeof(char));
			strcpy(toret->arr_newact[toret->nrnewact-1], $1);
			free($1);
		}
		;
...
newobs_param:	NEWOBSERS ':' newobs_val ;
newobs_val:	INTEGER			{
			/* initialize n observations with default mnemonics */
			toret->nrnewobs = $1;
			toret->arr_newobs = (char **) malloc($1 * sizeof(char *));
			int i;
			for (i=0; i<$1; i++) {   
				toret->arr_newobs[i] = (char *) malloc((nrdigit($1,16)+2) * sizeof(char));
				sprintf(toret->arr_newobs[i], "p%X", i);
			}
		}
		| newobs_list
		;
newobs_list:	newobs_list STRING		{
			/* add new observation with given mnemonic */
			toret->nrnewobs++;
			...
			toret->arr_newobs[toret->nrnewobs-1] = (char *) malloc((strlen($2)+1)*sizeof(char));
			strcpy(toret->arr_newobs[toret->nrnewobs-1], $2);
			free($2);
		}
		| STRING		{
			/* add new observation with given mnemonic */
			...
			free(toret->arr_newobs);
			toret->arr_newobs = tmpptr;
			toret->arr_newobs[toret->nrnewobs-1] = (char *) malloc((strlen($1)+1)*sizeof(char));
			strcpy(toret->arr_newobs[toret->nrnewobs-1], $1);
			free($1);
		}
		;
