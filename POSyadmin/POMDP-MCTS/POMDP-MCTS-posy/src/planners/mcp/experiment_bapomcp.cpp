...
void EXPERIMENTBAPOMCP::Run(std::vector<double>& Rhist)
{
     ...
     std::cout<<"mcts"<<std::endl;
      bool particle;
      for(episode = 0; episode<2000; episode++)
{
	...
	//std::cout<<"StepPOMDP"<<std::endl;
        ...
        // simstate = simstatenew;
				state = statenew;//update real state
	...
	delete[] counts1copy;
	time+=Results.Time.GetMean();
	cout << "(time--" << Results.Time.GetMean() << ")";
    //			<< Results.DiscountedReturn.GetMean() << ":"
	cout<<"time-mean:"<<time<<std::endl;	
}
/*
