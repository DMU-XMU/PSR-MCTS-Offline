...
int main(int argc, char* argv[]){
	...
	if(BAPOMCP){
		...
		timer.startTimer();
		//double *RESU =new double[K_episode];
		double RESU[K_episode] = {0};
		...
		for(uint i=0;i<N;++i){
			...
			expresult<<"the"<<i<<" ";
			Rhist[i].reserve(K_episode);
			experiment7.Run(Rhist[i]);
			for(int a=0;a<K_episode;a++){
			RESU[a]+=Rhist[i][a];
			expresult<<Rhist[i][a]<<" ";
			}
			//RESU[i] = mean;
		...
		for(int k=0;k<K_episode;k++)
		{
				RESU[k]=RESU[k]/N;
				meanresult << RESU[k] << std::endl;
				mean+=RESU[k];
		std::cout<<RESU[k]<<std::endl;
		//expresult<<RESU[k]/400<<" ";
		}
		//std::cout<<"mean--"<<sum/100<<std::endl;
		std::cout<<"mean:" << mean/K_episode<<std::endl;
		meanresult<<"mean: " << mean/K_episode<<std::endl;
		timer.stopTimer();
		std::string filename_bmcp(datafilename);
		filename_bmcp.append("_bpomcp");
		utils::dumpc(Rhist,filename_bmcp,K_episode);
		std::cout << "Time: " << timer.getElapsedTime()/(double)N  << " s";
		utils::append(timer.getElapsedTime()/(double)N,filename_bmcp_all);
	}
  if(BAUCT){
	...
	if(BEB){	
		...
		EXPERIMENT_BEB experiment6
		(*real,
         *real,filename_BEB_all,
				expParamsBEB,
				searchParamsBEB,
				*samplerFact);
		timer.startTimer();
