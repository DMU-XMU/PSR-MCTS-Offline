	...
	for(double x; in >> x;)
	{
		...
		for(uint s1=0;s1<S;s1++)
		{
			for(uint a=0;a<A;a++)
			{
				for(uint s2=0;s2<S;s2++)
				{
					countsOini[s1*S*A+a*S+s2]=Tnoisy[s1*S*A+a*S+s2]*20;   // -T
					//countsOini[s1*S*A+a*S+s2]=3;                            // -R
...
bool SYS::StepPOMDPsim(STATE& state, uint action, uint& observation, double& immediateReward)const
{
	...
	uint stateo = sysstate.position;
	uint statenew;
	utils::rng.multinom(T2+stateo*S*A+action*S,S,statenew);
	utils::rng.multinom(O2+statenew*O*A+action*O,O,observation);
	//std::cout << stateo <<" " <<  action << " " << T2[stateo*S*A+action*S] << std::endl;
